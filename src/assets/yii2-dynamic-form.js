...
(function ($) {
      ...
      $elem.find("div[data-dynamicform]").each(function () {
        ...
        for (var i = 1; i <= numItems - 1; i++) {
          ...
          currentWidgetOptions.fields.forEach(function (input) {
            ...
            if (
              $("#" + currentWidgetOptions.formId).yiiActiveForm("find", id) !==
              "undefined"
            ) {
  ...
  var _deleteItem = function (widgetOptions, e, $elem) {
    ...
    if (count > widgetOptions.min) {
      ...
      // trigger a custom event for hooking
      var eventResult = $("." + widgetOptions.widgetContainer).triggerHandler(
        events.beforeDelete,
        $todelete
      );
  ...
  var _fixFormValidatonInput = function (widgetOptions, attribute, id, name) {
    if (attribute !== undefined) {
      ...
      if (
        $("#" + widgetOptions.formId).yiiActiveForm("find", id) !== "undefined"
      ) {
        $("#" + widgetOptions.formId).yiiActiveForm("remove", id);
      }
      ...
      .each(function () {
        ...
        if (id !== undefined && name !== undefined) {
          currentWidgetOptions = eval(
            $(this).closest("div[data-dynamicform]").attr("data-dynamicform")
          );
          var matches = id.match(regexID);
          ...
          if (matches && matches.length === 4) {
            ...
            var attribute = $("#" + currentWidgetOptions.formId).yiiActiveForm(
              "find",
              baseID
            );
  ...
  var _restoreSpecialJs = function (widgetOptions) {
    ...
    if ($hasDatepicker.length > 0) {
      $hasDatepicker.each(function () {
        ...
        $(this)
          .parent()
          .datepicker(eval($(this).attr("data-krajee-kvdatepicker")));
      });
    }
    ...
    var _restoreKrajeeDepdrop = function ($elem) {
      var configDepdrop = $.extend(
        true,
        {},
        eval($elem.attr("data-krajee-depdrop"))
      );
    ...
    if ($hasSelect2.length > 0) {
      $hasSelect2.each(function () {
        ...
        $.when($("#" + id).select2(configSelect2)).done(
          initS2Loading(id, ".select2-container--krajee")
        );
